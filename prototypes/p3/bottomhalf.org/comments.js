...
for (var i=0; i < existingComments.length; i++) {
    ...
    displayComment(c.id, a.authorId, c.posted, c.comment);
    displayAuthor(c.user, a.name, a.avatar, a.siteCreator, a.homepage);
}*/


})();

// Give this comments.js script time to load the rest of this file
// Total HACK
setTimeout(function () {
...
var displayComment = function (id, authorId, posted, message) {

    if ($('#comment' + id).size() > 0) return;

    try{
  // TODO sometimes we'll get comments first, othertimes authors first...

  var d = new Date(posted);
  var aId = sanitize(authorId).replace(/,/g,'');
  console.log(aId);
  ...
  $('#bottom-half-comments-container').append(chtml);

  commentCounter++;
  $('#number-comments').text(commentCounter);
    }catch (e) { console.error(e.stack); console.error(e); }
};

function displayAuthor(id, name, avatarUrl, siteCreator, homepage) {
  // TODO sometimes we'll get comments first, othertimes authors first...

  var authorId = sanitize(id).toString().replace(/,/g, '');

  var chtml = '<img src="';
  chtml += avatarUrl + '" width="60" height="60" />';
  if (homepage) chtml += '<a href="' + homepage+ '">';
  chtml += '<span class="name">' + name + '</span>';
  if (homepage) chtml += '</a>';
  if (siteCreator) chtml += '<div class="site-creator">(Moderator)</div>';
  console.log('looping', '.author' + authorId);
  $('.author' + authorId + ' .a').each(function(el) {
    // TODO fixup date posted on, etc
    $(this).html(chtml);
  });
}
  ...
  var chan = Channel.build({
    window: document.getElementById("bh_comments_frame").contentWindow,
    origin: '*',
    scope: 'comments',
    onReady: function () {console.log('Channel is ready');}
  });

  console.log('Wiring up showCOmment');

  chan.bind('showComment', function(trans, comment) {
    var c = comment;
    displayComment(c.id, c.author, c.created, c.message);
    // Did we already have this author? They won't get showAuthor called again
    if (authors[c.author]) {
      var a = authors[c.author];
      displayAuthor(a.id, a.name, a.avatarUrl, a.siteCreator, a.homepage);
    }
    return true;
  });

  var authors = {};
  chan.bind('showAuthor', function(trans, author) {
